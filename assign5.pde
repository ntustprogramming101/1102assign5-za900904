...
void draw() {
	...
	switch (gameState) {
      ...
      if(isHit(playerX, playerY, SOIL_SIZE, SOIL_SIZE, cabbageX[i], cabbageY[i], SOIL_SIZE, SOIL_SIZE) ==false){  // r1 bottom edge past r2 top
        playerHealth ++;
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if(isHit(playerX, playerY, SOIL_SIZE, SOIL_SIZE, clockX[i], clockY[i], SOIL_SIZE, SOIL_SIZE) ==false){
        //gameTimer ++;
        addTime(gameTimer);
		...
		for(int i = 0; i < soldierX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
      if(isHit(playerX, playerY, SOIL_SIZE, SOIL_SIZE, soldierX[i], soldierY[i], SOIL_SIZE, SOIL_SIZE) ==false){
...
void drawTimerUI(){
  ...
  color timeTextColor = getTimeTextColor(gameTimer);
	...
	// Actual Time Text 		// Requirement #5: Get the correct color using color getTimeTextColor(int frames)
	fill(timeTextColor);
...
boolean isHit(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh){
  ...
  && bx < ax + bw    // r1 left edge past r2 right
...
String convertFramesToTimeString(int frames){	// Requirement #4
  ...
  if(frames/60>=60 && frames/60<119.1){
    mm = str(1);
...
void drawCaution(){								// Requirement #6
  for(int i=0; i<6;i++){
    float CautionExist = getEnemyIndexByRow(playerRow+5);
    if(CautionExist==i){
